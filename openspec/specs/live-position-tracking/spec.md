...
Drive live aircraft tracking on the CabinCrew tracking dashboard from a backend `flight.live-position-received` signal rather than flight status alone. Once that signal is seen — whether from the initial event history or delivered live over the flight events stream — the map polls the ADSB position feed throughout the active flight window (check-in through taxi-in), so tracking can begin during boarding instead of waiting for off-block, and stops once the flight is on-block. A solid map top bar states the ADS-B connection with a colour-plus-icon indicator alongside share controls; the flight-history map reuses the same bar titled "Historic flight path". The Operations overview map and the public flight map keep their existing status-based polling.
