...
The Flight Progress panel on the pilot tracking dashboard (`/track/:id`, Overview tab) is the pilot's primary situational readout and the control that advances a flight through its lifecycle. It names the current phase once and prominently, shows only the counters relevant to that phase in the app's data visual system, indicates during taxi-out whether takeoff will be reported automatically or must be reported manually, and exposes the autolock-guarded action that advances the flight.
