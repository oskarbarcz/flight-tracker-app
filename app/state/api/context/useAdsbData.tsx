...
const UseAdsbData = createContext<AdsbContextType>({
  callsign: null,
  flightPath: [],
  lastRequestedAt: null,
  setCallsign: () => {},
...
export const useAdsbData = () => {
  const ctx = useContext(UseAdsbData);
  if (!ctx) throw new Error("useAdsbApi must be used within a UseAdsbData");
  return ctx;
};
