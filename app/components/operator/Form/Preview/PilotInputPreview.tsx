  ...
  return (
    <div className="mt-2 flex items-center justify-between rounded-lg border border-gray-300 bg-gray-50 p-3 dark:border-gray-600 dark:bg-gray-700">
      <div>
        ...
        <span className="mt-1 block text-xs dark:text-gray-300">Email: {user.email}</span>
        <span className="text-xs dark:text-gray-300">License ID: {user.pilotLicenseId}</span>
      </div>
      <div>
        <Button onClick={() => onClose()} color="light" className="cursor-pointer">
          Remove
        </Button>
      </div>
    </div>
