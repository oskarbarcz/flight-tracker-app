...
**Display / Body Font:** Noto Sans (with `sans-serif` fallback).
...
### Hierarchy
- **Display** (Noto Sans 700, 1.875rem, 1.15): flight/page titles (e.g. the flight number header).
- **Headline** (Noto Sans 700, 1.25rem, 1.3): panel section headings ("Souls on board", "Fuel plan").
- **Title** (Noto Sans 600, 1rem, 1.4): card and group titles.
- **Body** (Noto Sans 400, 0.875rem/14px, 1.5): default UI text; cap prose at 65–75ch, though dense tables may run wider.
- **Label** (Noto Sans 700, ~0.6875rem/11px, uppercase, `letter-spacing: 0.08em`): the small tracked caps used for field labels and column keys ("PILOTS", "TRACKING", "EN-ROUTE & RESERVES").
